...
liste_url =["sante-bio","boissons-alcolisees","anti-gaspi-1","epicerie-salee-bio","epicerie-sucree-bio","vrac-bio","boissons-bio","maison-ecologique","beaute-hygiene-bio","bebe-bio-ecologique"]
...
for j in liste_url:
    ...
    if j == "boissons-alcolisees" or "anti-gaspi-1":
        variable_class = "3927393017"
    else :
        variable_class = "2564600132"
    ...
    for i in range(1,6):
        ...
        for i in product_datasheet:
            try:
                description = i.find("span", {"class":f"jsx-{variable_class}"}).get_text()
            except:
                description = "aucune"
            
            try :    
                marque = i.find("span", {"class":f"jsx-{variable_class} vendor"}).get_text()
            except:
                marque = "aucune"
            
            try :
                prix_unite = i.find("span", {"class":f"jsx-{variable_class} price-by-unit"}).get_text()
            except :
                prix_unite = "aucun"
            
            try:
                prix = i.find("span", {"class":f"jsx-{variable_class} price"}).get_text()
            except:
                prix = "aucun"
            
            try:
                prix_avant_reduc = i.find("span", {"class":f"jsx-{variable_class} compared-price"}).get_text()
            except:
                prix_avant_reduc = "aucun"
            
            try:
                pourcentage_reduc = i.find("div", {"class":f"jsx-{variable_class} discount-price"}).get_text()
            except:
                pourcentage_reduc = "aucun"
        
            try:
                quantite = i.find("span", {"class":f"jsx-{variable_class} weight"}).get_text()
            except:
            ...
            with open(f"{date}-données_la_fourche_tot.csv","a",encoding="utf-16") as fichier_extract:
